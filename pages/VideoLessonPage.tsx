...
const VideoLessonPage: React.FC = () => {
  ...
  const [completedLessons, setCompletedLessons] = useState<Set<string>>(new Set(['1-1']));
  ...
  const moduleInfo = courseId && moduleId ? courseModules[courseId]?.find(m => m.id === moduleId) : undefined;
  
  const lessonsData = (courseId && moduleId && courseLessons[courseId]?.[moduleId]) 
    ? courseLessons[courseId][moduleId]
    : courseLessons['default']['default-module'];
  
  const currentLessonIndex = lessonsData.lessons.findIndex(l => l.id === lessonId);
  const currentLesson = lessonsData.lessons[currentLessonIndex];
  
  const prevLesson = currentLessonIndex > 0 ? lessonsData.lessons[currentLessonIndex - 1] : null;
  const nextLesson = currentLessonIndex < lessonsData.lessons.length - 1 ? lessonsData.lessons[currentLessonIndex + 1] : null;
  ...
  const toggleComplete = (id: string) => {
    setCompletedLessons(prev => {
      ...
      if (newSet.has(id)) {
        // This could allow un-completing, for now we just add.
        // newSet.delete(id);
      } else {
        newSet.add(id);
      }
        ...
        <main className="flex-1 flex flex-col lg:flex-row overflow-y-auto">
          ...
          <div className="flex-grow bg-black transition-all duration-300 ease-in-out flex flex-col">
            ...
            <div className="p-4 sm:p-6 lg:p-8 flex-grow">
              ...
              <div className="mb-8">
                <h2 className="font-semibold text-white text-lg mb-2">Descrição da Aula</h2>
